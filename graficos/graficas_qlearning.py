"""Generate the Q-Learning learning curve for MountainCar-v0."""
...
DEFAULT_OUTPUT = Path(__file__).with_name("qlearning_curva_aprendizaje.png")
...
def plot_learning_curve(
    ...
    plt.plot(episodes, rewards_array, alpha=0.25, linewidth=0.8, label="Recompensa por episodio")
    ...
    if window > 1:
        ...
        plt.plot(average_episodes, average, linewidth=2, label=f"Media movil ({window} episodios)")

    plt.axhline(-200, color="tab:red", linestyle="--", linewidth=1, label="Limite: -200")
    plt.title("Curva de aprendizaje de Q-Learning en MountainCar-v0")
    plt.xlabel("Episodio")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20_000,
        help="Numero de episodios de entrenamiento (por defecto: 20000).",
    )
    parser.add_argument(
        "--window",
        type=int,
